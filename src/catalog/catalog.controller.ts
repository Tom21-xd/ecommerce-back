...
export class CatalogController {
  ...
  async createCategory(@Body() body: { name: string; slug: string; parentId?: number }, @Res() res: Response) {
  ...
  async addImage(@Body() body: { productId: number; base64: string; alt?: string; position?: number }, @Res() res: Response) {
    const result = await this.service.addImage(body.productId, body.base64, body.alt, body.position ?? 0);
    return res.status(HttpStatus.CREATED).json({ status: 201, message: 'created', result });
  }

  @Get('product-images')
  @ApiOperation({ summary: 'List product images' })
  async listImages(@Query('productId') productId: string, @Res() res: Response) {
  ...
  async addReview(@Req() req: Request, @Body() body: { productId: number; rating: number; comment?: string }, @Res() res: Response) {
    const user = req.user as any;
    const result = await this.service.addReview(user.id, body.productId, body.rating, body.comment);
    return res.status(HttpStatus.CREATED).json({ status: 201, message: 'created', result });
  }
